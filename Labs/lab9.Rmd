...
`ex_data` by `ID.
```{r}
a <- left_join(a, read_ex(dfiles, 2))
a
```
...
takes a folder name as its argument and 
    ...
    4. returns `ex_data`.
```{r}
read_ex_data <- function(name) {
    name <- dir(name,full.names=TRUE)
    ex_data <- read_ex(name,1)
    for(i in 2:length(name)) {
        ex_data <- left_join(ex_data, read_ex(name,i))
    }
    ex_data
}
read_ex_data("Lab9Data")
```
